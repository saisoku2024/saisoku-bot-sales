...
DECLARE
    ...
    v_daily_seq INT;
BEGIN
    -- 1. Check actor role in users table
    ...
    IF NOT FOUND THEN
        UPDATE public.pending_orders SET status = 'approved', approved_at = NOW() WHERE id = p_order_id;
        
        RETURN QUERY SELECT FALSE, 'Stok produk habis! Order ditandai sebagai Pending Delivery/Restock.'::TEXT,
                            p_order_id, NULL::UUID, v_order.user_id, v_order.product_id, v_order.qty, v_order.unit_price::NUMERIC, v_order.total_price::NUMERIC, v_order.telegram_id::BIGINT;
        RETURN;
    END IF;
    ...
    RETURN QUERY SELECT TRUE, 'Approval order berhasil.'::TEXT,
                        p_order_id, v_trx_id, v_order.user_id, v_order.product_id, v_order.qty, v_order.unit_price::NUMERIC, v_order.total_price::NUMERIC, v_order.telegram_id::BIGINT;
END;
$$;
